...
export const calculateCharge = (arrival: Date, departure: Date, chargeType: ChargeType): number => {
    ...
    if(departure < arrival) {
        throw "Departure date must be after arrival date.";
    }
...
const calculateLongStayCost = (arrival: Date, departure: Date): number => {
    var days = calculateChargeableDays(arrival, departure);
...
export const isAfterHours = (date: Date):boolean => {  
    return (getTimeOfDay(date) > parkingCalculatorConfig.closingTime);
} 

export const isBeforeHours = (date: Date):boolean => {  
    return (getTimeOfDay(date) < parkingCalculatorConfig.openingTime);
}
